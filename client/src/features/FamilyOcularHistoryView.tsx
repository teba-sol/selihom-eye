...
export const FamilyOcularHistoryView: React.FC = () => {
  const setFamilyOcularHistory = useEncounterStore((s) => s.setFamilyOcularHistory);
  const sectionData = useEncounterStore((s) => s.sectionData);
  const setSectionData = useEncounterStore((s) => s.setSectionData);

  const flags = Object.assign({}, DEFAULT_FLAGS, sectionData['family-ocular-history'] ?? {});
  ...
  return (
    <div className="p-8 max-w-4xl bg-white min-h-full">
      <h1 className="text-2xl font-bold text-[#2563eb] mb-6">Family Ocular History</h1>
