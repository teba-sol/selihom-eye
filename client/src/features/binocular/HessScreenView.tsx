...
export const HessScreenView: React.FC = () => {
  const [odData, setOdData] = useState<Record<string, string>>({});
  const [osData, setOsData] = useState<Record<string, string>>({});
  const [underaction, setUnderaction] = useState('');
  const [overaction, setOveraction] = useState('');
    ...
    <div className="p-8 max-w-4xl bg-white min-h-full">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {POSITIONS.map(pos => (
              <div key={pos}>
                ...
                <input type="number" value={odData[pos] ?? ''} onChange={e => setOdData(p => ({ ...p, [pos]: e.target.value }))}
                  placeholder="0"
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {POSITIONS.map(pos => (
              <div key={pos}>
                <label className="block text-[10px] text-slate-400 mb-0.5">{pos}</label>
                <input type="number" value={osData[pos] ?? ''} onChange={e => setOsData(p => ({ ...p, [pos]: e.target.value }))}
                  placeholder="0"
                  className="w-full px-2 py-1.5 text-xs text-center border border-slate-300 rounded bg-white font-semibold focus:outline-none focus:border-blue-600" />
              </div>
            ))}
      ...
      <div className="space-y-4 max-w-2xl mb-8">
        <Row label="Underaction">
          <input type="text" value={underaction} onChange={e => setUnderaction(e.target.value)} placeholder="e.g. Right Superior Oblique"
        ...
        <Row label="Overaction">
          <input type="text" value={overaction} onChange={e => setOveraction(e.target.value)} placeholder="e.g. Left Inferior Oblique"
            className="w-full px-3 py-2 text-sm border border-slate-300 rounded-md bg-white focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-200" />
        </Row>
      </div>
