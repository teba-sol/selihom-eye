...
type AccommodativeFacilityData = {
  method: string;
  od: string;
  os: string;
  ou: string;
...
const DEFAULT: AccommodativeFacilityData = {
  method: '',
  od: '',
  os: '',
  ou: '',
...
export const AccommodativeFacilityView: React.FC = () => {
  ...
  const f = Object.assign({}, DEFAULT, sectionData['accommodative-facility'] ?? {}) as AccommodativeFacilityData;
  const patch = (p: Partial<AccommodativeFacilityData>) => setSectionData('accommodative-facility', { ...f, ...p });
    ...
    <div className="p-8 max-w-4xl bg-white min-h-full">
      ...
      <div className="space-y-4 max-w-2xl mb-8">
        ...
        <Row label="Right Eye (OD) — cpm">
          <input type="number" value={od} onChange={e => patch({ od: e.target.value })} placeholder="e.g. 12"
        ...
        <Row label="Left Eye (OS) — cpm">
          <input type="number" value={os} onChange={e => patch({ os: e.target.value })} placeholder="e.g. 10"
            className="w-full max-w-lg px-3 py-2 text-sm border border-slate-300 rounded-md bg-white text-slate-800 font-semibold focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-200" />
        </Row>
        <Row label="Both Eyes (OU) — cpm">
          <input type="number" value={ou} onChange={e => patch({ ou: e.target.value })} placeholder="e.g. 8"
            className="w-full max-w-lg px-3 py-2 text-sm border border-slate-300 rounded-md bg-white text-slate-800 font-semibold focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-200" />
        </Row>
      </div>
