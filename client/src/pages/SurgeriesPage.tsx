...
type UnifiedDetails = Record<string, unknown>;

function getUnifiedDetails(s: SurgeryListItem): UnifiedDetails | null {
  const d = s.details as Record<string, unknown> | null | undefined;
  if (!d) return null;
  if (d.unifiedDetails) return d.unifiedDetails as UnifiedDetails;
  // Legacy: some records store the unified shape directly under details
  if (d.diagnosis || d.preOpVaOd || d.preOpVaOs) return d;
  return null;
}
...
function getClinicalData(s: SurgeryListItem): ClinicalData {
  const details = getUnifiedDetails(s) ?? {};
  const sfx = s.eye === 'OS' ? 'Os' : 'Od';

  const biometryOd = (details.biometryOd ?? {}) as Record<string, unknown>;
  const biometryOs = (details.biometryOs ?? {}) as Record<string, unknown>;

  const iolPower =
    strVal(biometryOd.iol) ||
    strVal(biometryOs.iol) ||
    strVal(details.iolPcOd) ||
    strVal(details.iolPcOs);

  const preOpVa =
    strVal(details[`preOpVa${sfx}`]) ||
    strVal(details.preOpVa);

  const postOpVa =
    strVal(details[`postOpDay1Va${sfx}`]) ||
    strVal(details.postOpVa);

  const diagnosis =
    strVal(details.diagnosis) ||
    strVal((s.details as Record<string, unknown> | null)?.diagnosis);
  ...
  return {
    ...
    postOpVa,
    diagnosis,
    surgeon: strVal(details.surgeon) || s.surgeon || '',
    dateOfSurgery: strVal(details.dateOfSurgery) || s.dateOfSurgery || '',
    eyeToBeOperated: strVal(details.eyeToBeOperated) || s.eye || '',
    iopOd: strVal(details.preOpIopOd),
    iopOs: strVal(details.preOpIopOs),
  };
}
      ...
      {objectEntries.map(([k, v]) => {
        ...
        return (
          <div key={k} className="mt-2">
            ...
            <div className="mt-1 rounded bg-white border border-slate-200 px-3 py-1">{rows}</div>
          </div>
        );
      })}
      {objectRows(scalar)}
...
export const SurgeriesPage: React.FC = () => {
  ...
  // Precompute clinical data per surgery once — avoids recomputation on every render
  const clinicalById = useMemo(() => {
    const map = new Map<string, ClinicalData>();
    for (const s of surgeries) map.set(s.id, getClinicalData(s));
    return map;
  }, [surgeries]);
  ...
  const filtered = useMemo(() => {
    ...
    return surgeries.filter((s) => {
      ...
      if (!q) return true;
      const cl = clinicalById.get(s.id);
      const haystack = `${s.patientName ?? ''} ${s.mrn ?? ''} ${s.type ?? ''} ${s.surgeon ?? ''} ${cl?.diagnosis ?? ''}`.toLowerCase();
      return haystack.includes(q);
    });
  }, [surgeries, searchQuery, statusFilter, clinicalById]);
  ...
  const viewingClinical = viewing ? clinicalById.get(viewing.id) ?? getClinicalData(viewing) : null;
  ...
  }, [viewing, viewingClinical, viewingDetails]);
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {paginated.map((s) => {
                    const cl = clinicalById.get(s.id) ?? getClinicalData(s);
